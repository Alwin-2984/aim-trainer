...
export default function TrackingTrainer() {
  ...
  useEffect(() => {
    if (state.phase === GamePhase.GAME_OVER && state.score > 0) {
      saveScore({ mode: 'tracking', difficulty, score: state.score, totalShots: 0, accuracy: null });
    }
...
function getOverlayConfig(phase: GamePhase, score: number, accentColor: string, difficulty: Difficulty, personalBest: number | null, isNewBest: boolean) {
  ...
  switch (phase) {
    case GamePhase.MENU:
      return { title: 'Smooth Tracking', description: TrackingModes[difficulty].description + pbLine, btnText: 'Lock Mouse & Play' };
