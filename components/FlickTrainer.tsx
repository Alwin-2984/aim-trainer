...
export default function FlickTrainer() {
  ...
  const { title, description, btnText } = getOverlayConfig(state.phase, state, settings.color, difficulty);
...
function getOverlayConfig(
  ...
  switch (phase) {
    case GamePhase.MENU:
      return {
        title: 'Micro-Flick',
        description: FlickModes[difficulty].description,
    ...
    case GamePhase.GAME_OVER: {
      ...
      const stats = `Score: <span style="color:${accentColor}; font-weight:800; font-size:1.2rem;">${state.score}</span> | Accuracy: <span style="color:${accentColor}; font-weight:800; font-size:1.2rem;">${accuracy}%</span><br>Time's up!`;
