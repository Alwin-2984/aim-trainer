...
export default function FlickTrainer() {
  ...
  const { recordingRef, createRecording, clearRecording } = useReplayRecording();
  ...
  useEffect(() => {
    ...
    const rec = createRecording('flick');
  ...
  return (
    <GameShell ref={shellRef} flashOpacity={flashOpacity}>
      <Overlay
        ...
        onSaveReplay={() => recordingRef.current && saveReplay({ recording: recordingRef.current, mode: 'flick', difficulty, score: state.score })}
